...
function usage() {
  ...
  echo ""
  echo "Note that this script will examine the file qdata1/numberOfNodes to"
...
performValidation istanbul-genesis1.json

mkdir -p qdata1/logs

numNodes=7
if [[ -f qdata1/numberOfNodes ]]; then
    numNodes=`cat qdata1/numberOfNodes`
fi
...
ARGS="--emitcheckpoints --nodiscover --nousb ${allowSecureUnlock} --verbosity ${verbosity} --istanbul.blockperiod ${blockPeriod} --networkid $NETWORK_ID --syncmode full --mine --miner.threads 1 --rpc --rpccorsdomain=* --rpcvhosts=* --rpcaddr 0.0.0.0 --rpcapi admin,eth,debug,miner,net,shh,txpool,personal,web3,quorum,istanbul,quorumPermission,quorumExtension --unlock 0 --password passwords.txt $QUORUM_GETH_ARGS"
basePort=21000
baseRpcPort=22000
...
do
    ...
    PRIVATE_CONFIG=ignore nohup geth --datadir qdata1/dd${i} ${ARGS} ${permissioned} --rpcport ${rpcPort} --port ${port} 2>>qdata1/logs/${i}.log &
    #PRIVATE_CONFIG=qdata1/c${i}/tm.ipc nohup geth --datadir qdata1/dd${i} ${ARGS} ${permissioned} --rpcport ${rpcPort} --port ${port} 2>>qdata1/logs/${i}.log &
done

set +v

echo
echo "All nodes configured. See 'qdata1/logs' for logs, and run e.g. 'geth attach qdata1/dd1/geth.ipc' to attach to the first Geth node."
