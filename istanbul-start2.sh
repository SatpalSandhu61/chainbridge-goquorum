...
function usage() {
  ...
  echo ""
  echo "Note that this script will examine the file qdata2/numberOfNodes to"
...
performValidation istanbul-genesis2.json

mkdir -p qdata2/logs

numNodes=7
if [[ -f qdata2/numberOfNodes ]]; then
    numNodes=`cat qdata2/numberOfNodes`
fi
...
do
    ...
    PRIVATE_CONFIG=ignore nohup geth --datadir qdata2/dd${i} ${ARGS} ${permissioned} --rpcport ${rpcPort} --port ${port} 2>>qdata2/logs/${i}.log &
    #PRIVATE_CONFIG=qdata2/c${i}/tm.ipc nohup geth --datadir qdata2/dd${i} ${ARGS} ${permissioned} --rpcport ${rpcPort} --port ${port} 2>>qdata2/logs/${i}.log &
done

set +v

echo
echo "All nodes configured. See 'qdata2/logs' for logs, and run e.g. 'geth attach qdata2/dd1/geth.ipc' to attach to the first Geth node."
